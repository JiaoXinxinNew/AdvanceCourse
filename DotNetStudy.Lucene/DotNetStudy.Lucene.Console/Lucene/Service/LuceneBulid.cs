using DotNetStudy.Lucene.Console.Lucene.Interface;
...
namespace DotNetStudy.Lucene.Console.Lucene.Service
{
    public class LuceneBulid : ILuceneBulid
    {
        ...
        /// </summary>
        /// <param name="activityList">sourceflag统一的</param>
        /// <param name="pathSuffix">索引目录后缀，加在电商的路径后面，为空则为根目录.如sa\1</param>
        /// <param name="isCreate">默认为false 增量索引  true的时候删除原有索引</param>
        public void BuildIndex(List<Activity> activityList, string pathSuffix = "", bool isCreate = false)
        {
            ...
            try
            {
                ...
                activityList.ForEach(c => CreateactivityIndex(writer, c));
            }
            finally
            {
                if (writer != null)
        ...
        {
            System.Console.WriteLine("MergeIndex Start");
            IndexWriter writer = null;
            try
            {
                if (childDirs == null || childDirs.Length == 0) return;
                Analyzer analyzer = new StandardAnalyzer(LuceneNet.Util.Version.LUCENE_20);
            ...
            finally
            {
                ...
                System.Console.WriteLine("MergeIndex End");
            }
        }
        ...
        public void InsertIndex(Activity activity)
        {
            IndexWriter writer = null;
            try
            {
                ...
                writer.MergeFactor = 100;//控制多个segment合并的频率，默认10
                writer.UseCompoundFile = true;//创建符合文件 减少索引文件数量
                CreateactivityIndex(writer, activity);
            }
            catch (Exception ex)
            {
        ...
        public void InsertIndexMuti(List<Activity> activityList)
        {
            BuildIndex(activityList, "", false);
        }
        ...
        /// </summary>
        /// <param name="activityList"></param>
        public void DeleteIndexMuti(List<Activity> activityList)
        {
            IndexReader reader = null;
            try
            {
                if (activityList == null || activityList.Count == 0) return;
                Analyzer analyzer = new StandardAnalyzer(LuceneNet.Util.Version.LUCENE_30);
                string rootIndexPath = StaticConstant.IndexPath;
                DirectoryInfo dirInfo = Directory.CreateDirectory(rootIndexPath);
                LuceneIO.Directory directory = LuceneIO.FSDirectory.Open(dirInfo);
                ...
                {
                    reader.DeleteDocuments(new Term("Id", activity.Id.ToString()));
                }
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="activity"></param>
        public void DeleteIndex(Activity activity)
        {
            IndexReader reader = null;
            try
            {
        ...
        ///// 更新一条数据的索引
        ///// </summary>
        //public void UpdateIndex(Activity activity)
        //{
        //    DeleteIndex(activity);
        //    InsertIndex(activity);
        //}
        ...
        public void UpdateIndex(Activity activity)
        {
            ...
            try
            {
                ...
                writer.MergeFactor = 100;//控制多个segment合并的频率，默认10
                writer.UseCompoundFile = true;//创建符合文件 减少索引文件数量
                writer.UpdateDocument(new Term("Id", activity.Id.ToString()), ParseactivitytoDoc(activity));
            }
            catch (Exception ex)
            {
                logger.Error("InsertIndex异常", ex);
        ...
        /// </summary>
        /// <param name="activityList">sourceflag统一的</param>
        public void UpdateIndexMuti(List<Activity> activityList)
        {
            IndexWriter writer = null;
            try
            {
        ...
        private PerFieldAnalyzerWrapper CreateAnalyzerWrapper()
        {
            ...
            analyzerWrapper.AddAnalyzer("categoryid", new StandardAnalyzer(LuceneNet.Util.Version.LUCENE_30));
        ...
        private void CreateactivityIndex(IndexWriter writer, Activity activity)
        {
            try
            {
                writer.AddDocument(ParseactivitytoDoc(activity));
            }
            catch (Exception ex)
            {
                logger.Error("CreateactivityIndex异常", ex);
                throw ex;
            }
        }
        ...
        /// <param name="activity"></param>
        /// <returns></returns>
        private Document ParseactivitytoDoc(Activity activity)
        {
